...
contract QExecutiveGovernance {
    ...
    QToken public secondaryToken; // it's used for chaining governance contracts

    event Execution(uint proposalID);
    event ExecutionFailure(uint proposalID);

    constructor(address governanceTokenAddress) public {//, address secondaryTokenAddress
    ...
    function addProposal(address destination, uint value, bytes memory data) public returns (uint) {
        ...
        p.electionExpirationTimestamp = now; //+ 5 * 1 minutes; disable for demo purposes
        p.exists = true;
    ...
    function executeTransaction(uint proposalID) public {
        ...
        if (isTransactionProposalConfirmed(proposalID)) {
            ...
            tp.executed = true;
            (bool ok,) = tp.destination.call.value(tp.value)(tp.data);
            if (ok)
    ...
    function checkVoteValidity(uint proposalID, bool approved) internal view {
        ...
        //require(p.electionExpirationTimestamp > now, "voting disabled after the expiration date"); disable for demo purposes
    ...
    function isTransactionProposalConfirmed(uint transactionProposalID) public view returns (bool){
        ...
        return tp.totalApproved > tp.totalDeclined; //tp.electionExpirationTimestamp < now && removed for demo purposes
    }
